...
| **G-6** | **Dominio `danzclass.com`:** apuntar a Vercel, actualizar `APP_URL`, y actualizar Site URL + Redirect URLs de Supabase. Hoy los enlaces de referido apuntan a `danzclass.com` y **todo lo demás** al dominio de Vercel (ver D-3). | ⚠️ Pendiente |
...
**Hueco de documentación encontrado en esta sesión: `QR_TOKEN_SECRET`.** `apps/web/src/lib/qrAttendance.ts` la exige para emitir y validar cualquier token QR de asistencia (feature de `054`, entrenamientos con cobro mensual de `068` incluidos) y **no aparece en la tabla de env vars de `CLAUDE.md`** ni en ningún registro de sesión anterior. No hay forma de saber desde el código si está configurada en Vercel — **verificar directamente en el dashboard antes del lanzamiento**, porque si falta, ningún QR se emite ni se valida (el escaneo de asistencia completo queda roto, en silencio, sin ningún error visible hasta que un profesor intente escanear a un alumno).

**Opcional, sin impacto si falta:** `EXPO_PUBLIC_WEB_URL` (mobile) — si no está seteada, cae al dominio de Vercel hardcodeado (`packages/shared/src/lib/webUrl.ts`); solo hace falta configurarla cuando se decida apuntar mobile a `danzclass.com` (ver D-3/G-6).
