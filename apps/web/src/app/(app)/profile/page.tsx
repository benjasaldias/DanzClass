...
export default async function ProfilePage() {
  ...
  const [{ data: profile }, tier, { count: followersCount }, { count: followingCount }] =
    await Promise.all([
  ...
  return (
    <div className="px-4 py-6 space-y-5">
      <div className="flex flex-col items-center text-center gap-3">
        ...
        <div className="flex gap-6 text-sm text-gray-500">
          <div className="text-center">
            <p className="text-lg font-bold text-gray-900">{followersCount ?? 0}</p>
            <p>seguidores</p>
          </div>
          <div className="text-center">
            <p className="text-lg font-bold text-gray-900">{followingCount ?? 0}</p>
            <p>siguiendo</p>
          </div>
        </div>
      </div>
      ...
      <div className="card divide-y divide-gray-100">
        <Link href="/profile/edit" className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50">
          <Settings className="h-5 w-5 text-gray-400" />
          <span className="text-sm font-medium text-gray-700">Editar perfil</span>
        </Link>
        {canTeach(tier) && (
          <Link href="/profile/payment-info" className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50">
