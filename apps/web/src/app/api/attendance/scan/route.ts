...
function reject(reason: RejectReason) {
  return NextResponse.json({ status: 'rejected', reason, message: REJECT_MESSAGES[reason] })
}
...
export async function POST(request: NextRequest) {
  ...
  const { data: row } = await (admin as any)
    ...
    .select(`
      ...
      class:classes!inner(id, teacher_id, title, type, status, date, time,
        ...
        duration_minutes, ends_at, ends_indefinitely),
  ...
  // 4. Pago confirmado (defensa en profundidad además del token activo).
  if (enrollment?.status !== 'confirmed') return reject('payment_not_confirmed')
