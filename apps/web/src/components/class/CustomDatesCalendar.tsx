...
export default function CustomDatesCalendar({ dates, onClose }: CustomDatesCalendarProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 px-4">
      <div className="w-full max-w-sm rounded-t-3xl sm:rounded-2xl bg-white dark:bg-dark-surface p-5 shadow-2xl">
        ...
        <div className="flex items-center justify-between px-1 py-2 mb-2">
          <button onClick={prevMonth} className="rounded-lg p-1.5 hover:bg-gray-100 dark:hover:bg-dark-surface2 transition-colors">
          ...
          <div className="text-center">
            ...
            {datesThisMonth > 0 && (
              <p className="text-xs text-brand-600 dark:text-brand-300">{datesThisMonth} clase{datesThisMonth !== 1 ? 's' : ''} este mes</p>
            )}
          </div>
          <button onClick={nextMonth} className="rounded-lg p-1.5 hover:bg-gray-100 dark:hover:bg-dark-surface2 transition-colors">
        ...
        <div className="grid grid-cols-7 mb-1">
          {DAY_HEADERS.map((h) => (
            <div key={h} className="text-center text-[11px] font-medium text-gray-400 dark:text-dark-text2/60 py-1">{h}</div>
          ))}
        </div>
        ...
        <div className="mt-4 flex items-center gap-2 text-xs text-gray-500 dark:text-dark-text2">
